...
int main()
{
    ...
    for(int i=0;i<incsv.size();i++){
        ...
        for(int i=0;i < Z_meaA.cols();i++){
            ...
            Z_meaA(1,i) = r_sig; //r
            Z_meaA(2,i) = (-fyf*sin(incsv[i].deltaf) + fx*cos(incsv[i].deltaf))/m; //ax
            Z_meaA(3,i) = (fyf*cos(incsv[i].deltaf) + fyr + fx*sin(incsv[i].deltaf))/m; //ay
        }
        ...
        for(int i=0;i < Z_meaB.cols();i++){
            ...
            Z_meaB(0,i) = (cf+cfd)*alphaf_sig; //Fyf
            Z_meaB(1,i) = (cr+crd)*alphar_sig; //Fyr
